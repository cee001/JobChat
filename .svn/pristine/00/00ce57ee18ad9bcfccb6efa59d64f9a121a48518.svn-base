...
<html>
        ...
        $(function(){  
			   ...
			   $.ajax({
			       ...
			       success:function(d){					  			
					    ...
					    var data2 = data1.entity;	
                            typeId=data2.homemalltype.hmtypeId;			        			
					        $("#meg1").val(data2.homemallTitle);	
					        $("#meg2").val(data2.homemallPrice);
					        $("#meg3").val(data2.homemallImg);
					        $("#meg4").val(data2.homemallParam);
					        $("#meg5").val(data2.homemallLink);	
					        var img =data2.homemallImg;
					        img=img.split(',')[0];  
		  ...
		  $.ajax({
			   ...
			   $(data2).each(function(index,rdata){	
			           ...
			           if(rdata.hmtypeId==typeId){
			              $("#meg10").html($("#meg10").html()+'<option value="'+rdata.hmtypeId+'"  selected>'+rdata.hmtypeName+'</option>');
			           
			           }else{
			           
			              $("#meg10").html($("#meg10").html()+'<option value="'+rdata.hmtypeId+'" >'+rdata.hmtypeName+'</option>');
			           }
        ...
        function showconr(){ 
		  ...
		  $.ajax({
		       ...
		       success:function(d){	
                       ...
                       $("#ljop1").html('<div style="font-size:16px;color:red;margin-top: 100px;text-align:center;">产品修改成功!<br>'
  ...
  <body>
    <input id="homemallId" type="hidden" name="homemallId" value="<%=request.getParameter("hid") %>">
