            ...
            if(sid==1){
             ...
             if(confirm("确定删除吗？")){
                $.ajax({
                       async:false,                
				       type:"post",                         
				       url:"com!doUpdatepublic.action?comid="+comid,	 
                         
				       data:"commentsInfostatus="+sid,	
				       success:function(d){	
				                    
		                          window.location.href="com!listpublicom.action?tRandStr="+Math.random();
  ...
  <body>
   ...
   <div id="dvmsgid">
    <table   border="1" bordercolor="#B2DFEE" align="center" style="border-collapse: collapse;width:100%;line-height: 25px;font-size:14px;" cellpadding="0" cellspacing="0">   
     ...
     <tr align="center" >
	     <td>${tc.publiccommentsId}</td>
	     <td>${tc.SUser.userNikename}</td>
	     <td align="left" style="padding: 3px">${tc.publiccommentsInfo}</td>
	     <td width="100px" >${tc.SPublicmessages.publicmessagesId}</td>
	     <td>${tc.publiccommentsTime}</td>
	     <td>
	        <c:if test="${tc.publiccommentsInfostatus==0}">
	          <a onclick="doUpdate(${tc.publiccommentsId},2)" href="" style="color:#104E8B">通过</a>&nbsp;
	          <a onclick="doUpdate(${tc.publiccommentsId},1)" href="" style="color:#104E8B">删除</a>
		    </c:if>       	
	     </td>     
     </tr>		
     </c:forEach>  
