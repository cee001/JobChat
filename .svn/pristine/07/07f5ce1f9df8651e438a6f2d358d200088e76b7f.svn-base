	     ...
	     function getdata(m){  
	           ...
	           $.ajax({   
				  ...
				  $(data2).each(function(index,rdata){
                     ...
                     var img=rdata.homemallImg;                      
                      img=img.split(',')[0];
                      
			          var strtitle=rdata.homemallTitle;                                   
				      var t1=strtitle;
		  	          var datatitle=t1;	
				      if(t1.length>28){
                     ...
                     var link=rdata.homemallLink;    
                     link= link.substring(0, 29);    		 
				     
				     $("#dmsgid1").html($("#dmsgid1").html()+'<div class="list_div1" style="width:80px;">'+rdata.homemallId+'</div>');
				     ...
				     +'<a onclick="changeif(1)" href="publicmsgcon.jsp?mid='+rdata.homemallId+'" target="tvc">&nbsp;'+strtitle+'</a></div></div>');
				     $("#dmsgid3").html($("#dmsgid3").html()+'<div class="list_div1">'+rdata.homemallPrice+'</div>'); 
				     $("#dmsgid4").html($("#dmsgid4").html()+'<div class="list_div1">'+rdata.homemallParam+'</div>');
				     $("#dmsgid5").html($("#dmsgid5").html()+'<div class="list_div1">'+link+'</div>'); 				     				     
				     $("#dmsgid6").html($("#dmsgid6").html()+'<div class="list_div1">'+rdata.homemallTime+'</div>'); 
				     $("#dmsgid7").html($("#dmsgid7").html()+'<div class="list_div1">'+rdata.homemalltype.hmtypeName+'</div>'); 
		          	 $("#dmsgid8").html($("#dmsgid8").html()+'<div class="list_div1"><a href="up_homemall.jsp?hid='+rdata.homemallId+'" onclick="changeif(1)" target="tvc">修改</a>&nbsp;<a href="#" onclick="doUpdate('+rdata.homemallId+')">删除</a></div>');
            ...
            if(confirm("确定删除吗？")){
                $.ajax({
				       ...
				       success:function(d){	
		                          ...
		                          window.location.href="list_homemall.jsp";
