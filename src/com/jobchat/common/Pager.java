...
public class Pager {
	...
	public static String getPaging(int pageNum,int pageSize,int count,String url){
    	...
    	pager+="到第<input id='n101' type='hidden' value="+lastPager+"><input id='n100' style='width:35px' type='text'>页 <a id='m100' onmousemove='gethref()' href=''>确定</a> " ;
    	return pager;
	}
	
	public static String getPaging1(int pageNum,int pageSize,int count,String url,int mid){
		String pager="";
    	if(pageNum<=1){
    		pager+="上一页";
    	}else{
    		pager+="<a href='"+url+"?p="+(pageNum-1)+"&sid="+mid+"'>上一页</a> ";
    	}    	
    	//���һҳҳ��
    	int lastPager=count/pageSize;
    	lastPager=(int) Math.ceil(count/pageSize);
    	...
    	if(pageNum>=lastPager){
    		...
    		pager+="<a href='"+url+"?p="+(pageNum+1)+"&sid="+mid+"'>下一页</a>("+pageNum+"/"+lastPager+")";
    	} 
    	return pager;
	}
