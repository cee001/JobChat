...
public class CompanyjobService {
    ...
    public JCompanyjob getJCompanyjobById(int id){
    	...
    	return (JCompanyjob)cd.getObject(JCompanyjob.class, id);
    }      
	...
	public int getcount(int cid){
		String hql="select count(*) from JCompanyjob where status = '0'";
		if(cid != -1){
			   hql += "and company_id = '"+cid+"'";
	    }			
	...
	public ArrayList<JCompanyjob> getCompanyjobByPageNum(int pageNum,int pageSize,int cid){
		...
		String hql="from JCompanyjob where status = '0'";
		if(cid != -1){
			   hql += "and company_id = '"+cid+"'";
	    }		
