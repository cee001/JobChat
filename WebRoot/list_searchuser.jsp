  ...
  <body >
    <table border="1" bordercolor="#B2DFEE" align="center" style="border-collapse: collapse;width:100%;line-height: 25px;font-size:15px;" cellpadding="0" cellspacing="0">
        ...
        <tr>     
         ...
         <tr align="center">
             <td>${requestScope.u.userOutid}</td>
             <td>${requestScope.u.userNikename}</td>
             <td>${requestScope.u.userPhone }</td>
             <td>${requestScope.u.userTime }</td>  
             <td> 
                <c:if test="${requestScope.u.userType==0}">
   				   <a onclick="changeif(1)" href="ur!showDetail.action?u.userId=${requestScope.u.userId }" target="tvc">详细</a> 
   			    </c:if>
                <c:if test="${requestScope.u.userType==1}">
   				   <a onclick="changeif(1)" href="ur!detailcompany.action?uid=${requestScope.u.userId }" target="tvc">详细</a> 
   				</c:if>               
                <c:if test="${requestScope.u.userStatus==1}">
   				   <a href="" onclick="doUpdate(${requestScope.u.userId },0)"><font style="color:red">恢复</font></a>
   				</c:if>
                <c:if test="${requestScope.u.userStatus==0}">
                              
   				   <a href="" onclick="doUpdate(${requestScope.u.userId },1)"> 冻结 </a>
   				</c:if>                                                                    
