...
<html>
  ...
  <body >
    ...
    <c:forEach var="u" items="${requestScope.al}">
         ...
         <tr align="center">
             <td>${u.JUser.userOutid}</td>
             <td>${u.JUser.userNikename}</td>
             <td>${u.userinfoTruename}</td>
             <td>${u.JUser.userPhone }</td>
             <td>${u.JUser.userTime }</td>
  
             <td>
               <a onclick="changeif(1)" href="ur!showDetail.action?u.userId=${u.JUser.userId }" target="tvc">详细</a> 


                 <c:if test="${u.JUser.userStatus==1}">
   				   <a href="" onclick="doUpdate(${u.JUser.userId },0)"><font style="color:red">恢复</font></a>
   				</c:if>
                <c:if test="${u.JUser.userStatus==0}">
                              
   				   <a href="" onclick="doUpdate(${u.JUser.userId },1)"> 冻结 </a>
   				</c:if>              
