...
<html>
  ...
  <body >
    ...
    <c:forEach var="c" items="${requestScope.al}">
         ...
         <tr align="center">
             <td>${c.JUser.userOutid}</td>
             <td>${c.JUser.userNikename}</td>
             <td>${c.truename}</td>
             <td>${c.JUser.userPhone }</td>
             <td>${c.JUser.userTime }</td> 
             <td>
                <a onclick="changeif(1)" href="ur!listjob.action?cid=${c.companyId }" target="tvc">招聘</a>
                <a onclick="changeif(1)" href="ur!detailcompany.action?uid=${c.JUser.userId }" target="tvc">详细</a> 
                <c:if test="${c.JUser.userStatus==1}">
   				   <a href="" onclick="doUpdate(${c.JUser.userId },0)"><font style="color:red">恢复</font></a>
   				</c:if>
                <c:if test="${c.JUser.userStatus==0}">                             
   				   <a href="" onclick="doUpdate(${c.JUser.userId },1)">冻结 </a>
   				</c:if>                                                                                 
             </td>  
          </tr>
    </c:forEach>
